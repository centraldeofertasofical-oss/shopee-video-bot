...
async def importar_historico():
    log.info(f"SESSION_STRING configurada? {'SIM' if SESSION_STRING else 'NÃO'}")
    log.info(f"Tamanho SESSION_STRING: {len(SESSION_STRING)}")
    log.info(f"N8N_WEBHOOK configurado? {'SIM' if N8N_WEBHOOK else 'NÃO'}")
